...
def word2Vec():
    W2V = {}
    W2V_file = csv.reader(open(".\data\gaozhong_english\W2V.csv","r",encoding = "utf-8"))
    h = 1
    for stu in W2V_file:
        W2V[stu[1]] = stu[2]
    #    if h % 1000 == 0 :
...
def vocabulary():
    yelp_json_path = "data/gaozhong_english/gaozhong_english_preprocessed.csv"
    vocab_path = yelp_json_path[0:-5] + "_vocab_reverse.pickle"
...
class HAN():
    ...
    def __init__(self, vocab_size, num_classes, embedding_size=128, hidden_size=50):
        ...
        with tf.name_scope('placeholder'):
            ...
            self.embedding_mat = tf.placeholder(tf.float32, [self.vocab_size, self.embedding_size], name='embedding_mat')
    ...
    def word2vec(self):
        with tf.name_scope("embedding"):
            vocab = vocabulary()
            W2V = word2Vec()
            embedding_array = np.random.normal(size=(len(vocab),self.embedding_size))
            for i in range(len(vocab)):
                if vocab[i] in W2V :
                    embedding_array[i] = W2V[vocab[i]].split(",")
# =============================================================================
#                 else:
#                     print("0")
#                     print(vocab[i])
# =============================================================================
            self.embedding_mat = tf.convert_to_tensor(embedding_array, dtype = tf.float32)
